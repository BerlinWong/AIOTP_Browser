...
export default {
        ...
        tooltip: {
          ...
          formatter: (params) => {
            ...
            seriesData.forEach((line, index) => {
              // console.log(line)
              const todayPrice = line.data[dataIndex]
              const yesterdayPrice = dataIndex > 0 ? line.data[dataIndex - 1] : todayPrice
              const priceChangePercentage = ((todayPrice - yesterdayPrice) / yesterdayPrice * 100).toFixed(2)
              ...
              if (todayPrice != null) {
                tooltipContent += `<div><span ${dotStyle}></span> &nbsp;&nbsp;&nbsp;&nbsp;指数名称：${line.name}&nbsp;&nbsp;&nbsp;&nbsp;指数点：${todayPrice.toFixed(2)}&nbsp;&nbsp;&nbsp;&nbsp;涨跌幅：${priceChangePercentage}% \t区间涨跌幅：${qjPriceChangePercentage}%</div>`
              }
            })
  ...
  methods: {
    ...
    initCharts () {
      ...
      this.$store.dispatch('graph/graph_line/getSyntheticSiegeIndexV2', {
